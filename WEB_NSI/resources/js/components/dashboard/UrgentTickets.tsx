    ...
    return (
        <div className="bg-[#111827]/50 backdrop-blur-sm border border-slate-800 rounded-xl p-6 flex flex-col h-full shadow-lg">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-[10px] font-bold text-white tracking-[0.15em] uppercase opacity-70 font-sans">Urgent Tickets</h2>
                <div className="bg-rose-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded shadow-[0_0_12px_rgba(244,63,94,0.4)]">{data.length} New</div>
            </div>
                ...
                {data.map((ticket, i) => (
                    <div key={i} className="group cursor-pointer border-b border-white/5 pb-4 last:border-0 hover:bg-white/5 rounded-lg p-3 transition-colors">
                        <div className="flex justify-between items-start mb-1.5">
                            ...
                            <span className="text-[9px] text-slate-500 font-bold uppercase">{Math.floor(Math.random() * 60)}m ago</span>
                        </div>
                        <p className="text-[10px] text-slate-500 line-clamp-1 mb-2 font-medium">Issue with {ticket.ticket_number} in {ticket.customer.tier} zone...</p>
                        <div className="flex justify-between items-center">
                            <span className={`text-[8px] font-black px-1.5 py-0.5 rounded-sm uppercase tracking-widest ${ticket.priority === 'High' ? 'bg-rose-500/10 text-rose-500' : 'bg-blue-500/10 text-blue-500'}`}>
                                {ticket.priority} • {ticket.ticket_number}
                            </span>
                        </div>
                    </div>
                ))}
